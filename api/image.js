...
/* Detects an image-generation intent even when the 🎨 toggle is off,
   in both English and common Swahili phrasing. */
function isImageRequest(text){
  return /\b(draw|paint|illustrate|sketch)\b|generate\s+(an?\s+)?(image|picture|photo|logo)|create\s+(an?\s+)?(image|picture|photo|logo)|make\s+(an?\s+)?(image|picture|photo|logo)|\bchora\b|\bpicha\b|tengeneza\s+picha|unda\s+picha/i.test(text||"");
}
