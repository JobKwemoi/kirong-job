...
function setupHeaderShelfButton() {
    ...
    freshButton.setAttribute("aria-label", "Open chats");
}
...
function renderShelves() {
    ...
    sortedChats.forEach(chat => {
        ...
        item.innerHTML = `
            ...
            <button class="kirongDeleteChat" type="button" title="Delete chat">🗑️</button>
        `;
        ...
        chatBox.innerHTML = `
            <div class="kirongWelcome">
                ...
                <div class="welcomeChips">
                    ...
                    <button onclick="quickFill('Generate image of ')">Generate Image</button>
                </div>
            </div>
        `;
    }
...
function renderMarkdown(text) {
    ...
    content = content.replace(/```([\s\S]*?)```/g, (_, code) => `<pre><code>${code.trim()}<button class="copyCodeBtn">📋 Copy</button></code></pre>`);
...
function setSendingState(state) { isSending = state; if (sendBtn) { sendBtn.disabled = state; sendBtn.style.opacity = state? ".6" : "1"; sendBtn.innerHTML = state? "⏳" : "➤"; } }
...
// Clear + Export + Location + File + Voice code yako iko hapa... imebaki vile vile
...
function injectShelvesStyles() {
    ...
    style.textContent = `
        #kirongShelvesOverlay{position:fixed;inset:0;background:rgba(0,0,0,.52);opacity:0;visibility:hidden;pointer-events:none;z-index:9998;transition:opacity.28s;backdrop-filter:blur(4px)}
        #kirongShelvesOverlay.open{opacity:1;visibility:visible;pointer-events:auto}
        #kirongShelves{position:fixed;top:0;left:0;bottom:0;width:min(340px,88vw);transform:translateX(-105%);transition:transform.38s cubic-bezier(.16,1,.3,1);z-index:9999;overflow-y:auto;background:rgba(9,9,18,.96);border-right:1px solid rgba(255,255,255,.1);backdrop-filter:blur(30px)}
        #kirongShelves.open{transform:translateX(0)}
       .kirongShelvesHeader{position:sticky;top:0;display:flex;align-items:center;justify-content:space-between;padding:18px;background:rgba(9,9,18,.9);border-bottom:1px solid rgba(255,255,255,.07)}
       ...
       .kirongChatItem{display:flex;align-items:center;gap:10px;min-height:52px;margin-bottom:6px;padding:9px 10px;border-radius:14px;color:#cbd5e1;cursor:pointer}
       .kirongChatItem.active{background:rgba(139,92,246,.18)}
       .kirongCopyBtn{position:absolute;top:8px;right:8px;background:rgba(255,255,255,.08);border:0;border-radius:8px;padding:4px 8px;cursor:pointer;opacity:0;transition:opacity.2s}
       .message:hover.kirongCopyBtn{opacity:1}
       .kirongRetryBtn{margin-top:8px;padding:6px 12px;background:rgba(239,68,68,.15);border:1px solid rgba(239,68,68,.3);color:#fca5a5;border-radius:8px;cursor:pointer}
       .kirongTyping{display:inline-block}.kirongTyping span{display:inline-block;width:8px;height:8px;border-radius:50%;background:#8b5cf6;margin:0 2px;animation:bounce 1.4s infinite}
       .kirongTyping span:nth-child(2){animation-delay:.2s}.kirongTyping span:nth-child(3){animation-delay:.4s}
        @keyframes bounce{0%,80%,100%{transform:scale(0)}40%{transform:scale(1)}}
       .message{animation:fadeInUp.3s ease;position:relative} @keyframes fadeInUp{from{opacity:0;transform:translateY(10px)}to{opacity:1}}
        #chatForm{background:rgba(20,20,20,.6);backdrop-filter:blur(16px);border-top:1px solid rgba(255,255,255,.08)}
       .welcomeChips{display:flex;gap:8px;margin-top:16px;flex-wrap:wrap}.welcomeChips button{padding:8px 12px;border:1px solid rgba(139,92,246,.3);background:rgba(139,92,246,.1);color:#c4b5fd;border-radius:20px;cursor:pointer}
        pre{position:relative}.copyCodeBtn{position:absolute;top:8px;right:8px;font-size:11px;padding:4px 8px}
        @media (prefers-reduced-motion: reduce){.message,.kirongTyping span{animation:none}}
    `;
